...
const CreateAlbum: React.FC<CreateAlbumProps> = ({ closeModal }) => {
    ...
    // Dropdown options
    const contentTypesDropDown = useMemo((): DropDownOption[] => {
        if (!contentTypes) return [];
        return contentTypes.map((contentType: ContentType): DropDownOption => ({
            value: contentType.id ?? '',
            label: contentType.name ?? ''
        }));
    }, [contentTypes]);

    const albumAlertMessageDropDown = useMemo((): DropDownOption[] => {
        if (!albumAlertMessages) return [];
        return albumAlertMessages.map((albumAlertMessage: AlbumAlertMessage): DropDownOption => ({
    ...
    // Use to build model
    const [albumAlertMessageSelectedOption, setAlbumAlertMessageSelectedOption] = useState<DropDownOption | null>(null);
    const [contentTypesSelectedOptions, setContentTypesSelectedOptions] = useState<DropDownOption[] | null>([]);

    const onChangeAlbumAlertMessageSelectedOption = (selectedOption: DropDownOption | null) => {
